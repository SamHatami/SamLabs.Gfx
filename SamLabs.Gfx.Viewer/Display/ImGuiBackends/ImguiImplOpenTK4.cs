...
public static unsafe class ImguiImplOpenTk4
{
    ...
    private class WindowCallbacks
    {
        ...
        public void Window_KeyUp(KeyboardKeyEventArgs e)
        {
            Window_Key(e, false);
        }

        public void Window_KeyDown(KeyboardKeyEventArgs e)
        {
            Window_Key(e, true);
        }
        ...
        public void Window_Key(KeyboardKeyEventArgs e, bool isPressed)
        {
            ...
            var imguiKey = TranslateKey(e.Key);
    ...
    public static void Shutdown()
    {
        ...
        if (WindowMap.TryGetValue(bd->WindowPtr, out var window)) RestoreCallbacks(window);
    ...
    public static void NewFrame()
    {
        ...
        if (currentTime <= bd->Time) currentTime = bd->Time + 1;
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    private static void Platform_SetWindowTitle(ImGuiViewportPtr viewport, nint name)
    {
        var vd = (ViewportData*)viewport.PlatformUserData;
        var window = WindowMap[vd->WindowPtr];
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    private static void Platform_SetWindowFocus(ImGuiViewportPtr viewport)
    {
        var vd = (ViewportData*)viewport.PlatformUserData;
        var window = WindowMap[vd->WindowPtr];
        window.Focus();
