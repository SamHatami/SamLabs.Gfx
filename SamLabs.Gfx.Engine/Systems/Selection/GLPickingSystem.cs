...
public class GLPickingSystem : RenderSystem
{
    ...
    public GLPickingSystem(EntityRegistry entityRegistry) : base(entityRegistry)
    {
    ...
    public override void Update(FrameInput frameInput, RenderContext renderContext)
    {
        if (_pickingEntity == -1)
        {
            ...
            ComponentRegistry.SetComponentToEntity(new PickingDataComponent(), _pickingEntity);
        }
        ...
        var selectableEntities = ComponentRegistry.GetEntityIdsForComponentType<SelectableDataComponent>();
        ...
        {
            var mesh = ComponentRegistry.GetComponent<GlMeshDataComponent>(selectableEntity);
            if (mesh.IsManipulator)
                continue;

            var modelMatrix = ComponentRegistry.GetComponent<TransformComponent>(selectableEntity).WorldMatrix;
    ...
    {
        var parentManipulator = ComponentRegistry.GetEntityIdsForComponentType<ActiveManipulatorComponent>();
        if (!parentManipulator.IsEmpty) //No active manipulator (no manipulator selected)
        {
            ...
            var childManipulators = ComponentRegistry.GetChildEntitiesForParent(parentManipulator[0], childBuffer);
            foreach (var childManipulator in childManipulators)
            {
                var mesh = ComponentRegistry.GetComponent<GlMeshDataComponent>(childManipulator);
                if (!mesh.IsManipulator)
                    continue;
                var modelMatrix = ComponentRegistry.GetComponent<TransformComponent>(childManipulator).WorldMatrix;
